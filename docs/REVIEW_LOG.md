...
- **类型**：架构改进
- **时间**：Phase 3 - 3.2 设计阶段
- **问题**：原始方案是攒够上下文再整体摘要（事后压缩），但工具调用参数（如 write_file 的完整文件内容）和工具结果（如 read_file 的完整文件内容）会从第一轮起就撑大上下文
- **原始方案**：3.2 摘要生成 + 3.3 阈值触发压缩（攒够再摘要）
- **用户意见**：每次交互有必要保留完整的工具调用参数吗？有没有能在每轮对话压缩又不影响大模型理解的方案？
- **讨论结论**：改为逐轮即时压缩——每轮工具调用完成后立即压缩非最近一轮的工具消息，保留操作语义（做了什么、结果如何），丢弃冗余内容（完整文件内容、长输出）。模型需要具体内容时可重新调 read_file。类似人类工作记忆机制。
- **最终修改**：3.2 从"摘要生成"改为"逐轮压缩器"，3.3 从"阈值压缩"改为"集成到 Agent Loop"，新增 compressor.py 模块
